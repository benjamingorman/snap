...
def main():
    """
    Parse CLI args and start the game.
    """
    logging.basicConfig(level=logging.DEBUG)

    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--decks", type=int, default=1, help="The number of decks to play with."
    )
    parser.add_argument(
        "--players", type=int, default=2, help="The number of players to simulate."
    )
    parser.add_argument(
        "--turn-delay",
        type=float,
        default=0.0,
        help="Amount of time to wait between turns.",
    )
    parser.add_argument(
        "--fast-ai",
        action="store_true",
    ...
    logging.info(
        "Simulating snap with %d deck(s) and %d players.", args.decks, args.players
    )
