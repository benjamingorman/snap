...
class SnapSimulator:
    ...
    def is_game_over(self) -> bool:
        ...
        """
        non_empty_down_piles = [stack for stack in self.player_card_down_piles if len(stack)]
    ...
    def turn_over_new_card(self) -> dict[int, Card]:
        ...
        player_whose_turn_it_is = players_still_in[self.turn_count % len(players_still_in)]

        cards = {}
        cards_message = []
        for (i, pile) in enumerate(self.player_card_up_piles):
            if i == player_whose_turn_it_is.id:
        ...
        print("Turn {}: (Player {}) {}  |  Up: {}  |  Down: {}".format(
            self.turn_count + 1,
            player_whose_turn_it_is.id,
            ' '.join(cards_message),
    ...
    def gather_responses(self, cards: dict[int, Card]) -> List[PlayerMessage]:
        ...
        for player in self.players:
            player.turn_queue.put_nowait(ServerMessage(message_type=ServerMessageType.TURN, cards=cards.values()))

        # Wait for a response from all players still in the game (SNAP or NO_SNAP)
        while len(responses) < len([player for player in self.players if player.still_in_game]):
    ...
    def resolve_snap_decision(self, cards: dict[int, Card], player_id: int):
        ...
        Either they were correct and they win some cards, or they were incorrect and they lose a card.
        """
    ...
    def resolve_snap_decision_correct(self, snap_indices: List[int], player_id: int):
        ...
        """
        victory_cards = self.player_card_up_piles[snap_indices[0]] + self.player_card_up_piles[snap_indices[1]]
        print(f"Player {player_id} called snap correctly and won {len(victory_cards)} cards.")
    ...
    def resolve_snap_decision_incorrect(self, player_id: int):
        ...
        other player.
        """
        if not len(self.player_card_down_piles[player_id]):
            # Their down pile is empty
            return
        card = self.player_card_down_piles[player_id].pop()
        other_players_still_in = set([player.id for player in self.players if player.still_in_game]) - set([player_id])
        ...
        print(f"Player {player_id} called snap incorrectly and sacrificed a card to player {player_to_receive_card}.")
    ...
    def mark_players_out(self):
        ...
        for (down_pile, player) in zip(self.player_card_down_piles, self.players):
            if player.still_in_game and not len(down_pile):
                print(f"Player {player.id} is out!")
                player.turn_queue.put_nowait(ServerMessage(message_type=ServerMessageType.PLAYER_OUT))
    ...
    def start_game(self):
        ...
        dealer_cards = sum([CardStack.new_full_deck() for _ in range(self.config.num_decks)], start=CardStack())
        dealer_cards.shuffle()
        logging.debug("%s", dealer_cards)

        logging.info("Dealing the cards...")
        self.player_card_up_piles = [CardStack() for _ in range(self.config.num_players)]
        self.player_card_down_piles = [CardStack() for _ in range(self.config.num_players)]
